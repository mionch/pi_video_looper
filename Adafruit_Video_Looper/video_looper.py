...
class VideoLooper(object):
    def __init__(self, config_path):
        ...
        self._video_player = self._load_player()
        self._presenter = self._load_presenter()
        self._players = [self._video_player, self._presenter]
        ...
        self._extensions_map = {player: player.supported_extensions() for player in self._players}
    ...
    def _load_presenter(self):
        """Load the configured slideshow player and return an instance of it."""
        module = self._config.get('video_looper', 'slide_player')
        return importlib.import_module('.' + module, 'Adafruit_Video_Looper') \
            .create_player(self._config)
    ...
    def _build_playlist(self):
        ...
        # Enumerate all movie/presentation files inside those paths.
        files = []
        added_filenames = []
        for player, extensions in self._extensions_map.iteritems():
            player_files = []
            for ex in extensions:
                for path in paths:
                    ...
                    # key from an OSX computerDaddy
                    player_files += ['{0}/{1}'.format(path.rstrip('/'), x) for x in os.listdir(path)
                                     if re.search('\.{''0}$'.format(ex), x, flags=re.IGNORECASE) and x[0] is not '.']

                    player_files = [filename for filename in player_files if filename not in added_filenames]
                    added_filenames += player_files
                ...
                files += [(filename, player) for filename in player_files]

        log = open('pi_video_files', 'w')
        log.writelines([str(f) + '\n' for f in files])
        log.close()
        # Create a playlist with the sorted list of movies.
        return Playlist(sorted(files, key=lambda f: f[0]), self._is_random)
    ...
    def run(self):
        ...
        # Main loop to play videos/slideshows in the playlist and listen for file changes.
        while self._running:
            # Load and play a new movie if nothing is playing.
            if self._player and not self._player.is_playing():
                file, player = playlist.get_next()
                if file is not None:
                    # Start playing the first available movie.
                    self._print('Playing movie: {0}'.format(file))
                    self._player = player
                    self._player.play(file, loop=playlist.length() == 1, vol=self._sound_vol)
